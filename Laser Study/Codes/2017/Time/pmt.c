...
TH1F* B = new TH1F("B", "Channel B",50,-5.,5.);
...
int n=0;
   ...
   string date = sdate + " " + stime ;
     ...
     ifstream f_Masked;
         ...
         f_Laser.open(f);
         f_Masked.open("masked_channels_341673_26nov2017.txt");
         int j=0;
         if(f_Laser){
          while(!f_Laser.eof())
           {
...
f_Laser.close();
    ...
    for(int m = 1; m < 65; m++)
    {
          ...
          A -> Fill(Las_constant[1][m-1][1]-Las_constant[1][m-1][4]);        //A1
           ...
           A -> Fill(Las_constant[2][m-1][1]-Las_constant[2][m-1][4]);         //A-1
          ...
          // Left Modules
           B -> Fill(Las_constant[1][m-1][2]-Las_constant[1][m-1][3]);         //B1
         
           // Right Modules


           B -> Fill(Las_constant[2][m-1][2]-Las_constant[2][m-1][3]);        //B-1
           ...
           D -> Fill(Las_constant[2][m-1][13]-Las_constant[2][m-1][14]);       //D-1
