...
f_Masked.open("masked_channels_313662.txt");
          ...
          while(std::getline(f_Masked,line))
            {
             ...
             if (line.compare(mask) ==0) 
               {  
                 ...
                 Las_constant[myPart_ctr][tmp_module-1][tmp_channel] = -100.-1000.*j ;
                 j--;
                 break;
               }
              else{ 
    ...
    // Fill histograms
       ...
       TH1F* A[30];  
         ...
         D[13] =   new TH1F("D-7", "Channel D_{-7}",50,-5.,5.);
...
grA ->Draw("APE");
//grA->SetTitle("; #eta; RMS of #DeltaPMT [Run 288060]");          //start
//grA->SetTitle("; #eta; RMS of #DeltaPMT [Run 300585]");          //middle
grA->SetTitle("; #eta; RMS of #DeltaPMT [Run 313662]");          //end
grA->GetYaxis()->SetRangeUser(0.,0.09);
grB ->Draw("PE");
grD ->Draw("PE");
