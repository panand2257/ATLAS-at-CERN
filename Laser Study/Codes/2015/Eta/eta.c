...
TF1 fmean("fmean","gaus",-5., 5.);
for(int i=0;i<30;i++){
 
A[i]->Fit("fmean");
meanA[i] = fmean.GetParameter(1);
rmsA[i] = fmean.GetParameter(2);
erreta[i] = 0.05;
if(i<10)
etaA[i] = (0.05 + 0.1*i);
...
B[i]->Fit("fmean");
meanB[i] = fmean.GetParameter(1);
rmsB[i] = fmean.GetParameter(2);
...
D[i]->Fit("fmean");
meanD[i] = fmean.GetParameter(1);
rmsD[i] = fmean.GetParameter(2);
erretaD[i] = 0.1;
}
...
TGraphErrors* grA = new TGraphErrors(30,etaA,meanA,erretaD,nullptr); 
TGraphErrors* grB = new TGraphErrors(28,etaB,meanB,erretaD,nullptr); 
TGraphErrors* grD = new TGraphErrors(13,etaD,meanD,erretaD,nullptr);

//TGraphErrors* grA = new TGraphErrors(30,etaA,rmsA,erretaD,nullptr); 
//TGraphErrors* grB = new TGraphErrors(28,etaB,rmsB,erretaD,nullptr); 
//TGraphErrors* grD = new TGraphErrors(13,etaD,rmsD,erretaD,nullptr);
...
grA ->Draw("APE");
//grA->SetTitle("; #eta; Average Drift (%) [Run 265497]");         //start
//grA->SetTitle("; #eta; Average Drift (%) [Run 277650]");         //middle
grA->SetTitle("; #eta; Average Drift (%) [Run 287886]");         //end
//grA->GetYaxis()->SetRangeUser(-0.2,0.8);
grB ->Draw("PE");
grD ->Draw("PE");
