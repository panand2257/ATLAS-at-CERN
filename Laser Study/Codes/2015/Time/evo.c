...
4370.512,
  ...
  graph->SetName("");
   ...
   graph->Draw("AFB1");
   graph->GetXaxis()->SetRangeUser(x[0],x[50]);
...
graph->GetYaxis()->SetLabelColor(ci-3);
graph->GetYaxis()->SetTitleColor(ci-3);
...
grA->GetXaxis()->SetLimits(x[0],x[50]);
...
grA->Draw("AY+PE");
//grA->GetYaxis()->SetRangeUser(0.1,1.5);
