 ...
 while(!f_Cesium2.eof())
	    ...
	    f_Cesium2 >> tmp_channel;
          ...
          while(std::getline(f_Masked,line))
           ...
           Las_constant[myPart_ctr][tmp_module-1][tmp_channel] = (Cesium_constant[myPart_ctr][tmp_module-1][tmp_channel] -d)/Cesium_constant[myPart_ctr][tmp_module-1][tmp_channel]   ;
                  }
        }
     f_Masked.clear();
     f_Masked.seekg(0, ios::beg);
...
grA ->Draw("APE");
//grA->SetTitle("; #eta; Mean of (Cs - Las) Drift (%) [IOV1]");
grA->SetTitle("; #eta; Mean of (Cs - Las)/Cesium  [IOV2]");

//grA->GetYaxis()->SetRangeUser(0.,5.0);
grB ->Draw("PE");
grD ->Draw("PE");
